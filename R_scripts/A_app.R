...
CGMAP_PATH = "A_app/Apar_feb_assembly_merged_initial_emseq.CGmap.gz"
ORGANISM_FASTA_PATH = "A_app/Apar_genome.fasta"
width_of_context = 5
FASTA_INDEX_PATH = "A_app/Apar_genome_pUC19_lambda.fasta.fai"
...
export_cg_map(CG_BS_CG, "A_app/CG.CGmap")
export_cg_map(CG_BS_CGC, "A_app/CGC.CGmap")
export_cg_map(CG_BS_nonCGC, "A_app/nonCGC.CGmap")
...
saveRDS(CG_whole_genome_results, "A_app/CG_whole_genome_results.rds")
...
GENE_GTF_PATH <- "A_app/Amoebidium.chr.v3.gtf"
...
write.table(geneMethData, file = "A_app/A_app_gene_meth.tsv", quote = FALSE, row.names = FALSE, sep = "\t")

#Load in taxonomy data for genes:
taxDataPath <- "A_app/A_app_tax.tsv"
...
write.table(gene_taxonomies, file = "A_app/A_app_gene_taxonomic_categories.tsv", sep = "\t", row.names = FALSE, quote = FALSE)
categorySummary <- gene_taxonomies %>% group_by(domains) %>% summarise(A_app = n())
write.table(categorySummary, file = "A_app/A_app_categorySummary.tsv", sep = "\t", row.names = FALSE, quote = FALSE)
...
LGT_mCG_col <- gene_taxonomies[!is.na(gene_taxonomies$domains),] %>% group_by(domains) %>% summarise(total_mHCGD = sum(met_HCGD, na.rm = TRUE)/sum(cov_HCGD, na.rm = TRUE), count = n()) %>% ggplot(mapping = aes(x = domains, y = total_mHCGD*100, fill = domains)) +
  geom_col() +
...
LGT_mCG_col
ggsave("A_app/A_app_LGT_mCG_col.pdf", LGT_mCG_col, height = 10, width = 13, units = "cm")


LGT_mCG_bars <- ggplot(gene_taxonomies[!is.na(gene_taxonomies$domains),], mapping = aes(x = domains, y = mHCGD*100, fill = domains)) +
  geom_boxplot(outlier.shape = NA) + theme_bw() + scale_fill_manual(values = c("#E69F00", "#ab2320", "#ff6e6e", "#56B4E9", "#003f5c", "#2a6dad", "#8097ff")) +
  ylab("mHCGD (%)") + xlab("Domain of origin") + coord_cartesian(ylim=c(0, 100)) +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))
LGT_mCG_bars
ggsave("A_app/A_app_LGT_bars.pdf", LGT_mCG_bars, height = 10, width = 13, units = "cm")
...
LGT
ggsave("A_app/A_app_LGT.pdf", LGT, height = 15.14, width = 13.69, units = "cm")
...
data_grouped <- data_grouped %>%
  ...
  ungroup()
data_grouped
write.table(data_grouped, file = "A_app/A_app_methylated_category_enrichment.tsv", quote = FALSE, sep = "\t", row.names = FALSE, col.names = TRUE)
