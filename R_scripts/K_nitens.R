...
CGMAP_PATH = "K_nitens/GSM2801023_RL633_mC_K_nitens.CGmap.gz"
ORGANISM_FASTA_PATH = "K_nitens/120824_klebsormidium_Scaffolds_v1.0.fna"
width_of_context = 5
FASTA_INDEX_PATH = "K_nitens/120824_klebsormidium_Scaffolds_v1.0.fna.fai"
...
export_cg_map(CG_BS_CG, "K_nitens/K_nitens_CG.CGmap")
export_cg_map(CG_BS_CHG, "K_nitens/K_nitens_CHG.CGmap")
export_cg_map(CG_BS_CHH, "K_nitens/K_nitens_CHH.CGmap")
...
ggsave("K_nitens/K_nitens_context.pdf", contextPlot, units = "in", height = 2.5, width = 2)
...
GENE_GTF_PATH <- "K_nitens/171026_klebsormidium_v1.1.gff"
...
CHG_mcStats <- get_mc_stats_from_granges(BS = CG_BS_CHG, GR = transcripts)
CHH_mcStats <- get_mc_stats_from_granges(BS = CG_BS_CHH, GR = transcripts)
...
write.table(geneMethData, file = "K_nitens/K_nitens_gene_meth.tsv", quote = FALSE, row.names = FALSE, sep = "\t")

#Load in taxonomy data for genes:
taxDataPath <- "K_nitens/klebsormidium_tax_v1.1.txt"
taxData <- fread(taxDataPath, select = c(1, 11, 18, 19, 20, 21, 22, 23, 24))
taxData <- taxData[taxData$V23 != "Klebsormidium"]
...
write.table(gene_taxonomies, file = "K_nitens/K_nitens_gene_taxonomic_categories.tsv", sep = "\t", row.names = FALSE, quote = FALSE)
categorySummary <- gene_taxonomies %>% group_by(domains) %>% summarise(K_nit = n())
write.table(categorySummary, file = "K_nitens/K_nit_categorySummary.tsv", sep = "\t", row.names = FALSE, quote = FALSE)
...
ggsave("K_nitens/K_nitens_LGT_bars_CG.pdf", LGT_mCG_bars, height = 10, width = 13, units = "cm")
ggsave("K_nitens/K_nitens_LGT_bars_CHG.pdf", LGT_mCHG_bars, height = 10, width = 13, units = "cm")
ggsave("K_nitens/K_nitens_LGT_bars_CHH.pdf", LGT_mCHH_bars, height = 10, width = 13, units = "cm")
...
CG_LGT <- ggplot(CG_totalGeneTaxonomyFrequency, mapping = aes(x = CG_methylationStatus, y = frequency_proportion*100, fill = domains)) +
...
CHG_LGT <- ggplot(CHG_totalGeneTaxonomyFrequency, mapping = aes(x = CHG_methylationStatus, y = frequency_proportion*100, fill = domains)) +
  geom_col() + theme_bw() + scale_fill_manual(values = c("#E69F00", "#ab2320", "#ff6e6e", "#56B4E9", "#003f5c", "#2a6dad", "#8097ff")) +
  ylab("Proportion (%)") + xlab(paste0("Methylation status (>", cutOff*100, "% mCHG)"))
CHG_LGT

CHH_LGT <- ggplot(CHH_totalGeneTaxonomyFrequency, mapping = aes(x = CHH_methylationStatus, y = frequency_proportion*100, fill = domains)) +
  geom_col() + theme_bw() + scale_fill_manual(values = c("#E69F00", "#ab2320", "#ff6e6e", "#56B4E9", "#003f5c", "#2a6dad", "#8097ff")) +
  ylab("Proportion (%)") + xlab(paste0("Methylation status (>", cutOff*100, "% mCHH)"))
CHH_LGT
ggsave("K_nitens/K_nitens_CG_LGT.pdf", CG_LGT, height = 15.14, width = 13.69, units = "cm")
ggsave("K_nitens/K_nitens_CHG_LGT.pdf", CHG_LGT, height = 15.14, width = 13.69, units = "cm")
ggsave("K_nitens/K_nitens_CHH_LGT.pdf", CHH_LGT, height = 15.14, width = 13.69, units = "cm")
...
CHG_data_grouped <- CHG_data_grouped %>%
  ...
  ungroup()
CHG_data_grouped
write.table(CHG_data_grouped, file = "K_nitens/K_nit_methylated_category_enrichment.tsv", quote = FALSE, sep = "\t", row.names = FALSE, col.names = TRUE)
